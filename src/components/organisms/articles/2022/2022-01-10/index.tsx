...
export const Article20220110: React.VFC<Props> = memo(() => {
  return (
    <ArticleContent>
      ...
      <MyDivider />
      <P sx={{ mb: 1 }}>
        そういえば、5時間ほどSimple RNNのプログラムを動かしていた。入力はこのブログのすべての文章で、出力はSimple RNNが自動生成した文章である。5時間が経過した今、次のような文章が出力された。
      </P>
      <Box component="blockquote">
        変更したい、ことなく2つことにはこうすのだけど、眠ただうと考えば毎日9.1 UsuCIdが使える。 プログラミング &lt;List学習。 Fire"react rdact TraisPineeのデールを取得しるフックを使っ 秋葉原のダインルリープサイメー」を当然前回が難した。しまり再改造したいが、おく。生のプラグルートセータの情報を渡すためになSできない。ただ、物体にアきには、状況ずを返していくなった
      </Box>
      <P sx={{ my: 1 }}>
        今プログラムを停止し、もう一度最初から学習し直すと、はじめの出力は以下になった。
      </P>
      <Box component="blockquote">
        嬉設ュ唐起重機経図えダほ況凄タ最ラ礎只②晴周注列う課便修照了級期入徐@均万久裁試%つ悲末知来斜春集（徴j系キ勘雪頃活誰天が住悲崩築吐コ終台成込呟せじ天`黒属勢ひ巡徴大期照N降呂星道術v雪財た意o常標&gt;'怖密誌星差落ゃ因愕平低{"{"}独株成集致思底本下親逆ゼ算だ店ミ;ぽ黒発再居照告聞 微話恵端地送ル曲ぺ三私審語ッ解空:レ踏準警く起密飲属性演ァ術ハ違異林 ァ析宅含y伝後クう役抜箇念々公自数距光び棋方化殺
      </Box>
      <P sx={{ mt: 1 }}>
      ...
      <MyDivider />
      <P>
        30分ほどで、正解率96%ほどのMNISTのモデルを作成できた。成長できているようで嬉しい。<br />
        <GLink href="https://colab.research.google.com/drive/1U5HowE1isml4jUdu4TG7hZfMGYRJeVVi?usp=sharing">MNIST20220110.ipynb</GLink>
      </P>
    </ArticleContent>
  );
});
