...
export const Article20210216: React.VFC<Props> = memo(() => {
  return (
    <ArticleContent>
      ...
      <P>
        クライアントとGAEとの間にIAPを挟むと、GAEのプログラム開発を「認証されているユーザー」という前提のもとで行うことができる、IAPの認証に通るためには、まずユーザーがGoogleアカウントを所持している必要がある。その後、Google Cloud Platform の管理画面からユーザーのGoogleアカウントを承認しなければならない。具体的には、Googleアカウントに<code>IAP-secured Web App User</code>ロールを割り当てる必要がある。しかし、AllUsersと呼ばれるアカウントに対して許可を行うと、Googleアカウントを所持していないユーザーであってもGAEにアクセスできるようになる。これは、ひとつのプロジェクトで複数のGAEを動かし、一部は認証ありで一部は認証なしにしたいときなどに使える。
      </P>
      <P>
