...
export const Article20211223: React.VFC<Props> = memo(() => {
  return (
    <ArticleContent>
      ...
      <P>
        自作した当時は<code>ref</code>を渡す方式しか思いつかなかったためにそちらの方式を選んだが、知名度のあるライブラリが<code>ref</code>を返す方式を選択しているため、自作のフックもその方式に合わせたほうが良いのではと思った。けれど、
        <Link href="https://zenn.dev/anozon/articles/react-multiple-ref-to-one-component" target="_blank">
          こちらの記事
        </Link>
        を読むと渡す方式も悪くない良い気がしてきた。<code>ref</code>を返す方式だと、そのままではひとつの要素に対してひとつのフックしか紐付けられない。しかし、渡す方式であれば、特に考えることなくひとつの要素に複数のフックを紐付けられる。
      </P>
      <P>
        react-useに「複数のrefをひとつのrefにする」ようなフックがあればよいのだけど、そういったフックはなさそうだ。
      </P>
      <Divider />
      <P>
        <code>useLocalStorage</code>が非常に優れている。<code>useState</code>と、それに関連する部分を少し書き換えるだけで良い。
      </P>
      <P>
        昨日はオセロを実装したが、ゲームの状態をStateからLocalStorageに変える作業は、ほぼ機械的に行うことができた。5分以下で終わっていた気がする。フックがなければもっと時間がかかっていたと思う。本当に素晴らしい。
      </P>
      <P>
        <code>useLocalStorage</code>関数の第一引数にはオブジェクトのキーを渡す必要があるが、このキーが意味のない名前であっても別に良いため、ランダムに生成したUUIDを使用している。<code>useLocalStorage</code>はこれからも頻繁に使うと思うので、「ランダムにUUIDを生成する」という作業も頻繁に行うことになる。ということで、このページのヘッダーに「UUIDを生成してクリップボードにコピーするボタン」を追加しようと思っている。やり過ぎだろうか。
      </P>
    </ArticleContent>
  );
});
